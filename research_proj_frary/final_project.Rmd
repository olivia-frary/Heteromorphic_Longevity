...
##### Models with larger subset of lifespan data

```{r models_dl, message=FALSE, warning=FALSE, include=FALSE}
...
Model 1 formula to investigate sex determination as a predictor

```{r}
m1$formula
```

Model 2 formula to make a complicated model of three predictors

```{r}
m2$formula
```

Model 3 formula, used stepAIC method to find "best" model
...
Possibly add in commentary on the mod_best summary and how it shows sex_determination with the lowest p-value.


### Step 5: Look at model predictions

##### Model predictions with larger subset of lifespan data
...
```{r shorter_easy_tree_fig, fig.width=8}
...
![BEAST tree faceted with larger differences removed to take a look at the smaller data](tree/facet_tree1.png)

While using MAFFT online, I saw that some of my sequences could have been reversed which could explain some of the weird relationships seen with the neighbor joined tree. Here is a quick tree made with bayesian methodology going through 1,000,000 and burnout at 250. The program used was beast.

![BEAST tree](tree/beast_tree.png)

![BEAST tree with natural log lifespan data plotted in gradient](tree/gradient_tree.png)
